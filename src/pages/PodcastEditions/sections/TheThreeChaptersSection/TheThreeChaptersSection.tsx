...
export const TheThreeChaptersSection = ({
  ...
  /* Removed unused isSingapore variable */
  ...
  return (
    <section className="relative w-full max-w-[1440px] min-h-[400px] md:min-h-[500px] lg:h-[1000px] bg-white pl-0 pr-4 sm:pr-6 md:pr-8 lg:pr-10 xl:pr-12 py-2 sm:py-3 md:py-4 lg:py-[8px] ml-0 mt-2 sm:mt-3 md:mt-4 lg:mt-6 mb-8 overflow-hidden" style={{ overflowX: 'hidden' }}>
      ...
      <div className="flex flex-col w-full items-center lg:items-start gap-2 relative lg:absolute top-auto lg:top-[478px] left-0 lg:left-[120px] text-center lg:text-left mb-6 lg:ml-0 lg:pl-0">
        <p className="relative w-full max-w-none lg:w-auto lg:absolute lg:left-0 [font-family:'Geist',Helvetica] font-medium text-[20px] sm:text-[24px] lg:text-[32px] leading-[28px] sm:leading-[32px] lg:leading-[36px] whitespace-normal lg:whitespace-nowrap">
          <span className="text-[#7cb403]">Pods scheduled for: </span>
          <span className="text-[#7cb403]">{editionName} </span>
        ...
        <div className="hidden lg:flex gap-6 w-full lg:w-auto">
          {episodes.map((episode) => (
            <article
              key={episode.id}
              onClick={() => setIsModalOpen(true)}
              className="relative w-full max-w-[400px] lg:w-[424px] h-[320px] lg:h-[372px] bg-[#f8f8f8] rounded-[20px] lg:rounded-[28px] overflow-hidden transition-all duration-300 hover:shadow-lg hover:scale-105 hover:bg-white cursor-pointer group"
            >
              <div
                className="inline-flex items-center gap-2 lg:gap-2.5 p-2 lg:p-3 absolute top-4 lg:top-6 left-4 lg:left-6 bg-[#7bb302] rounded-[40px] group-hover:scale-110 transition-transform"
              ...
              <a
                href="#"
                className="absolute top-6 lg:top-9 right-4 lg:right-6 w-5 h-5 lg:w-6 lg:h-6 opacity-40 group-hover:opacity-100 transition-opacity"
      ...
      {decorativeImages.map((image) => (
        <div key={image.id} className={`${image.containerClass} pointer-events-none md:hidden lg:flex`} aria-hidden="true">
